...
export class GlobeMap {
  ...
  private async initGlobe(): Promise<void> {
    ...
    console.debug('[GlobeMap] initGlobe container dimensions', {
    ...
    console.debug('[GlobeMap] globe initialized', {
      globe: !!this.globe,
  ...
  private flushMarkersImmediate(): void {
    ...
    try {
      ...
      console.debug('[GlobeMap] flushMarkersImmediate', { markerCount: markers.length, enabledLayers: this.layers });
      if (!markers.length) {
        console.debug('[GlobeMap] flushMarkersImmediate: marker list is empty');
      }
      if (markers.length > 0) {
  ...
  private flushPolygons(): void {
    ...
    console.debug('[GlobeMap] flushPolygons', { polygonCount: polys.length, enabledLayers: this.layers });
  }
